...
De kennisgraaf draait op de **docker-LXC** (Proxmox, Portainer-endpoint 3) met de data **lokaal** op
...
```bash
curl -s -u <user>:<wachtwoord> http://192.168.10.23:7200/rest/repositories | jq -r '.[].id'  # inning
curl -s -o /dev/null -w '%{http_code}\n' http://192.168.10.23:8004/mcp                       # 401 = goed
```

Wil je de graaf van buiten bereikbaar maken (bijvoorbeeld voor een MCP-client), maak dan in
nginx-proxy-manager een host `graphdb-mcp.ipalm.nl` → `192.168.10.23:8004` met
`proxy_buffering off;`.
...
- **jij** → `graphdb.ipalm.nl`, GraphDB's eigen loginscherm. Zet daar géén basic-auth in
...
**2. vzdump, dagelijks 03:30** — de Proxmox-back-upjob draait met `--all` en pakt LXC 103 dus
automatisch mee, met retentie `keep-daily=7, keep-weekly=4` naar `synology-backup`. De volgorde is
bewust: de dump van 03:00 ligt er al en lift mee naar de NAS.
