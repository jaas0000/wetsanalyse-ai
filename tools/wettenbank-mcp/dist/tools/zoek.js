...
export async function handleZoek(args) {
    ...
    const queryDelen = [];
    if (titel)
        queryDelen.push(`overheidbwb.titel any "${titel}"`);
    if (rechtsgebied)
        queryDelen.push(`overheidbwb.rechtsgebied == "${rechtsgebied}"`);
    if (ministerie)
        queryDelen.push(`overheid.authority == "${ministerie}"`);
    if (regelingsoort)
        queryDelen.push(`dcterms.type == "${regelingsoort}"`);
