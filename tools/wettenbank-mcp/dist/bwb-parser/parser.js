...
function mapAttrName(name) {
    if (name in ATTR_REMAP)
        return ATTR_REMAP[name];
 ...
 * - <li.nr> → linr
 */
function extractTextMetadata(el) {
    const result = {};
    for (let i = 0; i < el.childNodes.length; i++) {
        ...
        if (tag === "kop") {
            ...
            for (let j = 0; j < childEl.childNodes.length; j++) {
                ...
                if (kopChild.nodeType !== 1)
                    continue;
                const kopChildEl = kopChild;
 ...
 */
function parseContentNodes(el, bwbId, path) {
    const items = [];
    for (let i = 0; i < el.childNodes.length; i++) {
        const child = el.childNodes.item(i);
        if (child.nodeType === 3) {
 ...
 */
export function parseElement(el, bwbId, parentPath, siblingIndex = 0) {
    const tag = el.tagName;
    const type = normalizeType(tag);
 ...
 */
function parseChildren(el, bwbId, parentPath) {
    const children = [];
    const counts = {};
    for (let i = 0; i < el.childNodes.length; i++) {
        const child = el.childNodes.item(i);
 ...
 */
export function parseBwbXml(xml, bwbId) {
