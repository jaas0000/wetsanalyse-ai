...
import { escapeerRegex, bouwTermPatroon, parseZoekterm, stripXml, zoekTermInArtikelDom, extraheerDocMetadata, parseRecords, parseXmlDoc, dedupliceerOpBwbId, haalWetstekstOp, sruRequest, domParser, xmlCache, } from "./index.js";
// ── Helpers ──────────────────────────────────────────────────────────────────
function makeSruXml(records) {
...
function makeRecord({ bwbId = "BWBR0002320", titel = "Algemene wet inzake rijksbelastingen", type = "wet", ministerie = "Financiën", rechtsgebied = "Belastingrecht", geldigVanaf = "2024-01-01", geldigTot = "9999-12-31", gewijzigd = "2024-07-19", locatie = `https://repository.example.nl/bwb/${bwbId}/current.xml`, } = {}) {
...
describe("dedupliceerOpBwbId", () => {
    function maakRegeling(bwbId, geldigVanaf) {
        return {
            bwbId, geldigVanaf,
...
describe("haalWetstekstOp", () => {
    ...
    it("stuurt standaard vandaag als peildatum mee (bug 2)", async () => {
        ...
        const eersteAanroep = fetchMock.mock.calls[0][0];
    ...
    it("gooit een fout als repository fetch een uitzondering gooit (netwerkfout)", async () => {
        ...
        vi.stubGlobal("fetch", vi.fn().mockImplementation(() => {
            if (call++ === 0)
                return Promise.resolve({ ok: true, status: 200, text: () => Promise.resolve(sruXml) });
