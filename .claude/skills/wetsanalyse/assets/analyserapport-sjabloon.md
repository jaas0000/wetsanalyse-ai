# Wetsanalyse — [citeertitel], [artikel(en)]

> Analyse volgens de methode Wetsanalyse (Ausems, Bulles & Lokin), activiteit 2 + 3.
> Dit is een **concept-analyse als hulpmiddel**: bedoeld voor multidisciplinaire validatie
> (jurist, informatieanalist, ICT). Interpretatiekeuzes zijn als zodanig gemarkeerd.

## 0. Bron en afbakening

- **Wet / regeling:** [citeertitel] ([type])
- **BWB-id:** [BWBR…]
- **Artikel(en):** [nr(s)] — pad: [hoofdstuk > afdeling > artikel]
- **Versie-/peildatum:** [YYYY-MM-DD]
- **Bronreferentie(s):** [jci…]
- **Analysefocus / hoofdvraag:** [indien opgegeven, anders: "volledige analyse van het artikel"]
- **Reikwijdte:** [welke leden zijn geanalyseerd; wat valt buiten scope]
- **Geraadpleegde definitie-/aanpalende artikelen:** [bv. art. 1 (begripsbepalingen), …]

## 1. Wettekst (letterlijk)

> Letterlijk overgenomen uit de bron, lid voor lid. Geen parafrase.

**Lid 1.** […]

**Lid 2.** […]

## 2. Activiteit 2 — Juridische structuur

### 2a/2b — Markeringen en classificaties

| # | Formulering (letterlijk) | JAS-klasse | Vindplaats | Toelichting (waarom deze klasse; evt. alternatief) |
| --- | --- | --- | --- | --- |
| 1 | "…" | Rechtssubject | lid 1 | … |
| 2 | "…" | Rechtsbetrekking | lid 1 | … |
| … | | | | |

### Samenhang rond de centrale klassen

> Beschrijf hoe de klassen samenhangen rond de rechtsbetrekking(en) en het/de rechtsfeit(en):
> wie is rechthebbende, wie plichthebbende, welk rechtsobject, door welk rechtsfeit ontstaat
> de betrekking, en onder welke voorwaarden.

- **Rechtsbetrekking(en):** […]
- **Rechtsfeit(en):** […]
- **Centrale rechtssubjecten/-objecten:** […]

## 3. Activiteit 3 — Betekenis

### 3a — Begrippen

| Begripsnaam | Klasse | Definitie (bron of [interpretatie]) | Voorbeeld | Kenmerken / relaties | Vindplaats | Twijfel/aanname |
| --- | --- | --- | --- | --- | --- | --- |
| … | … | … | … | … | … | … |

### 3b — Afleidingsregels

#### [naam regel] — [beslis-/reken-/specialisatieregel]

- **Uitvoervariabele:** […]
- **Invoervariabelen:** […]
- **Parameters:** [naam — waarde — geldigheidsperiode — bron]
- **Voorwaarden:** […]
- **Formulering:**
  ```
  [gestructureerde pseudo met expliciete operatoren]
  ```
- **Vindplaats / bron:** [artikel + lid + bronreferentie]
- **Twijfel/aanname:** […]

## 4. Reviewlog en aandachtspunten voor validatie

### Reviewlog

> Uitkomst van de twee iteratieve review-checkpoints (zie de skill). Per activiteit: het
> aantal reviewrondes en, per ronde, de op grond van de feedback doorgevoerde wijzigingen
> (of dat de analist meteen akkoord ging).

- **Activiteit 2:** [aantal rondes] — [akkoord in ronde 1 | per ronde: samenvatting van
  verwerkte feedback]
- **Activiteit 3:** [aantal rondes] — [akkoord in ronde 1 | per ronde: samenvatting van
  verwerkte feedback]

### Aandachtspunten voor multidisciplinaire validatie

> Wat een mens nog moet bevestigen voordat deze analyse de uitvoering in gaat.

- **Interpretatiekeuzes:** [classificaties of definities waarover redelijkerwijs verschil
  van inzicht mogelijk is]
- **Open normen / vage termen:** [die nadere invulling (uitvoeringsbeleid) vragen]
- **Openstaande delegaties:** [verwijzingen naar amvb/ministeriële regeling die nog
  geanalyseerd moeten worden; subdelegatie bij 'bij of krachtens']
- **Aannames:** [wat is aangenomen bij gebrek aan informatie]
- **Buiten scope:** [wat bewust niet is geanalyseerd]
